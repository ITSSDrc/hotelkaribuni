...
const salleFormSchema = z.object({
  ...
  status: z.enum(['Disponible', 'Réservée', 'En maintenance']),
});

type SalleFormValues = z.infer<typeof salleFormSchema>;

interface EditSalleFormProps {
  onFinished?: () => void;
  initialData?: any;
}
...
export default function EditSalleForm({ onFinished, initialData }: EditSalleFormProps) {
  ...
  const onSubmit = (data: SalleFormValues) => {
    if (!firestore) {
      toast({
        variant: 'destructive',
        title: 'Erreur',
    ...
    if(initialData) {
        ...
        const salleDocRef = doc(firestore, 'salles', initialData.id);
        setDoc(salleDocRef, data, { merge: true })
            .then(() => {
                toast({
                    title: 'Salle modifiée !',
                    description: `La salle "${data.name}" a été mise à jour.`,
                });
                onFinished?.();
            })
            .catch((serverError) => {
        ...
        addDoc(sallesCollectionRef, data)
          .then(() => {
            toast({
              title: 'Salle ajoutée !',
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Ex: Salle de Conférence 'Horizon'" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea
                  placeholder="Décrivez la salle, ses équipements, sa vue, etc."
        ...
        <FormField
          ...
          render={() => (
            <FormItem>
              ...
              <FormDescription>
                Gérez la galerie d'images pour cette salle.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <div className="flex justify-end pt-4">
            <Button type="submit" disabled={form.formState.isSubmitting}>
                ...
                {form.formState.isSubmitting 
                    ...
                    : (initialData ? 'Modifier la salle' : 'Ajouter la salle')}
            </Button>
        </div>
      </form>
    </Form>
